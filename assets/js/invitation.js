(() => {
  ...
  $('copyAddress').addEventListener('click',()=>copy('서울특별시 구로구 새말로 97 신도림테크노마트 8층 웨딩시티 아모르홀','예식장 주소를 복사했어요.'));
  ...
  $('shareKakao').addEventListener('click',async()=>{if(preview){notify('배포 후 기존 청첩장 주소로 공유돼요.');return;}try{const Kakao=await loadKakao();if(!Kakao.isInitialized())Kakao.init(config.kakaoKey);Kakao.Share.sendDefault({objectType:'feed',content:{title:'도훈 ♥ 영현, 우리의 행복한 겨울에 초대합니다',description:'2026.12.05 토요일 오후 3:40 · 신도림 웨딩시티 아모르홀',imageUrl:new URL('assets/images/share.jpg',config.siteUrl).href,link:{mobileWebUrl:config.siteUrl,webUrl:config.siteUrl}},buttons:[{title:'청첩장 보기',link:{mobileWebUrl:config.siteUrl,webUrl:config.siteUrl}}]});}catch{if(navigator.share){try{await navigator.share({title:'도훈 · 영현 결혼식에 초대합니다',url:config.siteUrl});return;}catch(e){if(e.name==='AbortError')return;}}await copy(config.siteUrl,'청첩장 링크를 복사했어요. 카카오톡에 붙여 넣어 주세요.');}});
